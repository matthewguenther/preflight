...
const SCOPE_RADIUS_NM = 10;
const LOW_ALT_FT = 3500;
const MAP_ZOOM = 11;
const TILE_SIZE = 256;
const CENTER = { lat: 36.3444, lon: -94.2211 };
const METERS_PER_NM = 1852;
const RANGE_RINGS_NM = [2.5, 5, SCOPE_RADIUS_NM];

function lonLatToWorld(lat, lon, zoom = MAP_ZOOM) {
...
const CENTER_WORLD = lonLatToWorld(CENTER.lat, CENTER.lon);
const METERS_PER_PIXEL = (156543.03392 * Math.cos((CENTER.lat * Math.PI) / 180)) / 2 ** MAP_ZOOM;

function pointFor(ac) {
  const world = lonLatToWorld(ac.lat, ac.lon);
  return {
    x: world.x - CENTER_WORLD.x,
    y: world.y - CENTER_WORLD.y,
  };
}

function rangePixels(rangeNm) {
  return (rangeNm * METERS_PER_NM) / METERS_PER_PIXEL;
}

function runwayPixels() {
  return Math.max(12, (6006 * 0.3048) / METERS_PER_PIXEL);
}
...
function MapTiles() {
  const centerTileX = Math.floor(CENTER_WORLD.x / TILE_SIZE);
  const centerTileY = Math.floor(CENTER_WORLD.y / TILE_SIZE);
  const tiles = [];
  ...
  return (
    <div className="absolute inset-0 z-0">
      {tiles.map((tile) => {
        const left = tile.x * TILE_SIZE - CENTER_WORLD.x;
        const top = tile.y * TILE_SIZE - CENTER_WORLD.y;
        return (
          <img
            key={`${tile.x}-${tile.y}`}
            alt=""
...
export function TrafficScope({ title = 'Traffic scope', className = '', compact = false }) {
  const traffic = useTraffic(SCOPE_RADIUS_NM);
  ...
  return (
    ...
    >
      <div className={`grid gap-4 ${compact ? 'xl:grid-cols-[1.25fr_1fr]' : 'lg:grid-cols-[1.55fr_1fr]'}`}>
        <div className={`traffic-scope relative overflow-hidden rounded-lg border border-cyan-300/20 bg-slate-950/80 ${compact ? 'min-h-[300px]' : 'min-h-[330px]'}`}>
          <MapTiles />
          ...
          <div className="absolute bottom-2 left-2 z-40 rounded border border-cyan-300/25 bg-slate-950/80 px-2.5 py-1.5 text-xs shadow-[0_0_18px_rgba(103,232,249,0.14)]">
            <div className="font-bold uppercase tracking-[0.2em] text-cyan-50">KVBT - Thaden Field</div>
            <div className="mt-0.5 text-[10px] uppercase tracking-[0.16em] text-cyan-300/70">RWY 18/36 - 6,006 ft</div>
          </div>
          ...
          <div className="absolute left-1/2 top-1/2 z-20 flex -translate-x-1/2 -translate-y-1/2 flex-col items-center">
            <div className="traffic-runway-icon" style={{ height: `${runwayPixels()}px` }} title="Runway 18/36 scaled to map zoom" />
            <div className="mt-1 text-[9px] font-bold uppercase tracking-[0.2em] text-cyan-50 drop-shadow-[0_0_8px_rgba(6,182,212,0.85)]">KVBT</div>
          </div>
          {RANGE_RINGS_NM.map((range) => (
            <div
              key={range}
              className="absolute left-1/2 top-1/2 z-10 rounded-full border border-cyan-300/20"
              style={{
                width: `${rangePixels(range) * 2}px`,
                aspectRatio: '1',
          ...
          {aircraft.slice(0, 28).map((ac) => {
            const point = pointFor(ac);
            ...
            return (
              <div
                key={ac.hex}
                className={`absolute z-30 -translate-x-1/2 -translate-y-1/2 ${tone}`}
          ...
          {!aircraft.length ? (
            <div className="absolute inset-0 flex items-center justify-center text-sm text-cyan-100/65">No ADS-B aircraft within {SCOPE_RADIUS_NM} NM.</div>
          ) : null}
        </div>
        <div className="flex flex-col gap-3">
          ...
          <div className={`${compact ? 'max-h-[184px]' : 'max-h-[238px]'} overflow-y-auto rounded-md border border-cyan-300/15`}>
            <table className="w-full text-left text-xs">
              ...
              <tbody>
                {aircraft.slice(0, 12).map((ac) => (
                  <tr key={ac.hex} className="border-t border-cyan-300/10">
                    <td className="px-3 py-2 font-mono font-semibold">
                      <a
                        className="text-cyan-50 underline decoration-cyan-300/35 underline-offset-4 hover:text-cyan-200"
                        href={aircraftProfileUrl(ac)}
                        target="_blank"
                        rel="noreferrer"
                        title={`Open aircraft details for ${ac.registration || ac.callsign}`}
                      >
                        {ac.callsign}
                      </a>
                    </td>
                    ...
                    <td className="px-3 py-2 text-cyan-100/75">{ac.track_deg ?? '--'} deg</td>
                  </tr>
                ))}
              </tbody>
            </table>
